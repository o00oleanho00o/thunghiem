...
import { rotatedFootprint } from "@cabinet/model/geometry";
import { renderToSvg, contentWidth } from "@cabinet/render/toSvg";
...
export default function App() {
  ...
  }, [model, selected, issues, zoom, exportState, timings, dragObserved]);
  ...
  const doReflow = () => {
    ...
    let next = model;
    for (const item of result) next = updateElement(next, item.id, { x_mm: +item.x_mm.toFixed(2) });
    set(next);
  };
