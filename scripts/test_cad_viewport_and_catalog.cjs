...
(async () => {
  ...
  try {
    ...
    const component = page.locator('[data-component-id="device-001"]'); await component.dispatchEvent('dblclick'); assert.ok(await page.evaluate(() => window.CNB_APP.state.zoom >= 3.5), JSON.stringify(await page.evaluate(() => ({ zoom: window.CNB_APP.state.zoom, selected: window.CNB_APP.state.selectedId })))); await page.keyboard.press('F'); assert.equal(await page.evaluate(() => window.CNB_APP.state.zoom), 1); assert.equal(await page.evaluate(() => window.CNB_APP.state.panX), 0);
    ...
    const beforeCount = await page.evaluate(() => window.CNB_APP.state.model.components.length); await page.locator('.asset-card').first().click(); assert.equal(await page.evaluate(() => window.CNB_APP.state.model.components.length), beforeCount + 1); assert.ok(await page.evaluate(() => Boolean(window.CNB_APP.state.model.components.at(-1).assetId)));
