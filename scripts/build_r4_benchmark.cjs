...
const chosenIds = ['source-d2e35ab9238f5738', 'source-cd095c0d1e21645e', 'source-a0a9f79675c3f78e', 'source-a65546c5c7347b1c', 'source-7ab1640d1d97df20'];
const records = chosenIds.map((id) => catalog.records.find((item) => item.source_asset_id === id)).filter(Boolean);
records.forEach((record) => { const vector = vectorManifest.records.find((item) => item.source_asset_id === record.source_asset_id); record.vector_cache_ref = vector ? `catalog/generated/vector-cache/${record.source_asset_id}.json` : null; });
const cacheRoot = path.join(root, 'catalog/generated/vector-cache');
function model(label) {
  return { schemaVersion: 'eir-0.1', units: 'millimetres-declared-preview', project: { id: 'r4-real-dxf-composer', name: label, revision: 'A' }, enclosure: { id: 'generic-engineering-enclosure', name: 'Generic engineering enclosure · preview only', width: 1600, height: 1100, depth: 300 }, mountingPlate: { id: 'plate-main', x: 50, y: 50, width: 1500, height: 1000 }, rails: [{ id: 'rail-top', x: 90, y: 850, length: 1420, height: 7.5 }, { id: 'rail-mid', x: 90, y: 600, length: 1420, height: 7.5 }, { id: 'rail-low', x: 90, y: 350, length: 1420, height: 7.5 }], ducts: [{ id: 'duct-left', x: 55, y: 100, width: 28, height: 800 }, { id: 'duct-right', x: 1517, y: 100, width: 28, height: 800 }], components: [], connections: [], metadata: { r4PreviewMode: true, previewCadAssets: true, authoritative: false, knownUnitOnly: true, truthNote: 'Only source DXFs with explicit millimetre units are shown; identity and physical mapping are unresolved.' } };
}
function addComponents(target, yOffset) {
  records.forEach((record, index) => {
    const bbox = record.source_bbox;
    target.components.push({ id: `r4-${index + 1}`, tag: `-CAD${index + 1}`, name: record.candidate_description, kind: 'cad_asset', manufacturer: record.manufacturer || 'Siemens candidate', partNumber: `CANDIDATE-${record.source_asset_id}`, group: record.product_family || 'Siemens candidate', width: bbox.width, height: bbox.height, depth: 1, x: 100 + (index % 4) * 350, y: yOffset - (index >= 4 ? 250 : 0), rotation: 0, railId: null, mounting: 'Plate', locked: false, assetId: record.source_asset_id, source_asset_id: record.source_asset_id, cadGeometryRef: record.vector_cache_ref, metadata: { geometryStatus: 'source_verified', identityStatus: 'candidate_needs_review', physicalMappingStatus: 'unknown', sourceHash: record.sha256, sourceUnits: 'millimetres', placementCapable: false } });
  });
}
...
function enriched(candidate) { const copy = JSON.parse(JSON.stringify(candidate)); copy.components.forEach((component) => { const cachePath = path.join(root, 'catalog', 'generated', 'vector-cache', `${component.assetId}.json`); const cache = JSON.parse(fs.readFileSync(cachePath, 'utf8')); const record = records.find((item) => item.source_asset_id === component.assetId); component.cadGeometry = cache.geometry; component.cadGeometryBounds = record.source_bbox; component.cadGeometryScale = 1; }); return copy; }
