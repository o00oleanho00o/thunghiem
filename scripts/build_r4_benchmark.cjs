...
for (const [name, candidate] of [['r4-preview', regenerated]]) { fs.writeFileSync(path.join(out, 'exports', `${name}.dxf`), exportDxf(enriched(candidate))); fs.writeFileSync(path.join(out, 'exports', `${name}.svg`), exportSvg(candidate)); }
