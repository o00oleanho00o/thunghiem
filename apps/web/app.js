(function bootstrap() {
  ...
  function assetApproved(asset) { return Boolean(asset && asset.review_state === 'approved-footprint' && asset.unit_confidence && asset.physical_width_mm > 0 && asset.physical_height_mm > 0); }
  ...
  function autoLayout() {
    const model = state.model; const plate = model.mountingPlate; const priority = (group) => ({ Incoming: 0, Power: 1, 'Motor starter': 2, Control: 3, General: 4, Terminals: 9 }[group] ?? (/^Feeder/.test(group) ? 2 : 5)); const ordered = [...model.components].filter((component) => !component.assetId || assetApproved(assetFor(component))).sort((a, b) => priority(a.group) - priority(b.group) || a.group.localeCompare(b.group) || (b.width * b.height) - (a.width * a.height) || a.id.localeCompare(b.id));
  ...
  function renderAssetDetail() {
    ...
    form.addEventListener('submit', (event) => { event.preventDefault(); const unit = form.elements.unit.value; const width = finite(form.elements.width.value, 0); const height = finite(form.elements.height.value, 0); const view = form.elements.view.value; if (asset.drawing_sheet) { showToast('Drawing sheet requires manual extraction before approval', 'error'); return; } if (!unit || unit !== 'mm' || width <= 0 || height <= 0 || !view) { showToast('Approval requires mm unit, physical bounds and a view', 'error'); return; } asset.unit_confidence = 'confirmed-by-review'; asset.unit_source = 'reviewer'; asset.source_units = 'millimetres'; asset.source_units_code = 4; asset.physical_width_mm = width; asset.physical_height_mm = height; asset.candidate_view = view; asset.review_state = 'approved-footprint'; asset.approval = { approved_from_asset_id: asset.id, approved_at: new Date().toISOString(), review_source: 'web-review', unit_source: 'reviewer', view_source: 'reviewer' }; renderAssetLibrary(); showToast('Footprint approved with reviewer dimensions'); });
    $('#addPreviewAsset').addEventListener('click', () => addAssetComponent(asset, assetApproved(asset)));
  }

  function addAssetComponent(asset, approved) { const next = nextDeviceIndex(); const component = componentFrom('plc', next, state.model.enclosure.width / 2, state.model.enclosure.height / 2); component.name = asset.candidate_description || 'Imported CAD asset'; component.partNumber = `CAD-${asset.id}`; component.assetId = asset.id; component.footprintRef = `cad-asset:${asset.id}`; component.mounting = 'Plate'; if (approved && assetApproved(asset)) { component.width = asset.physical_width_mm; component.height = asset.physical_height_mm; } else { component.width = 75; component.height = 100; } component.x = snap(state.model.enclosure.width / 2 - component.width / 2, GRID_MM); component.y = snap(state.model.enclosure.height / 2 - component.height / 2, GRID_MM); state.model.components.push(component); state.selectedId = component.id; validate(); render(); showToast(approved ? 'Approved CAD footprint added to panel' : 'Preview-only asset added; excluded from authoritative layout'); }
