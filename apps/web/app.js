(function bootstrap() {
  ...
  function renderProductLibrary() {
    ...
    $$('.asset-card', list).forEach((item) => item.addEventListener('click', () => {
      ...
      const detail = $('#assetDetail'); detail.hidden = false; detail.innerHTML = `<h3>${esc(product.manufacturer_part_number)}</h3><dl><div><dt>Manufacturer</dt><dd>${esc(product.manufacturer)}</dd></div><div><dt>Series</dt><dd>${esc(product.series)}</dd></div><div><dt>Description</dt><dd>${esc(product.description)}</dd></div><div><dt>Envelope</dt><dd>${product.footprint.width_mm} × ${product.footprint.height_mm} × ${product.footprint.depth_mm} mm</dd></div><div><dt>Mounting</dt><dd>${esc(product.footprint.mounting)}</dd></div><div><dt>Provenance</dt><dd>${esc(product.source_url)}</dd></div><div><dt>Status</dt><dd>${esc(product.verification_status)}</dd></div></dl><span class="asset-badge approved">R3 ${esc(product.verification_status.toUpperCase())}</span>`;
    }));
  }
