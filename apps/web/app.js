(function bootstrap() {
  ...
  function r4Scenario() {
    // These are existing Siemens vector-cache assets. Their source geometry is
    // shown at source-unit scale for inspection; no asset is treated as an
    // authoritative mm footprint because the catalog has no exact mapping.
    const selected = [
      ['source-a0a9c0419df569bb', 'S7-1200 CPU candidate', 23.81, 95.25, 'SIMATIC S7-1200'],
      ['source-67d0e317f239396c', 'S7-1200 PM1207 candidate', 23.81, 47.63, 'SIMATIC S7-1200'],
      ['source-33c324f4df97c631', 'S7-1200 SM1221 candidate', 23.81, 95.25, 'SIMATIC S7-1200'],
      ['source-fe6d18f6b8ce400b', 'S7-1500 SM531 candidate', 23.81, 142.88, 'SIMATIC S7-1500'],
      ['source-e2454df537a9505c', 'S7-1500 CPU candidate', 23.81, 73.74, 'SIMATIC S7-1500'],
      ['source-5438331b54de9954', 'S7-1500 technology I/O candidate', 23.81, 111.13, 'SIMATIC S7-1500'],
      ['source-66748a98dbbdcf49', 'SIRIUS contactor candidate', 297, 210, 'SIRIUS'],
      ['source-a0a9f79675c3f78e', 'SINAMICS G120C candidate', 677.6, 349.25, 'SINAMICS'],
    ];
    const model = baseModel('R4 Real DXF Composer', 1600, 1100);
    model.metadata = { r4PreviewMode: true, previewCadAssets: true, authoritative: false, datasetNote: 'Real catalog geometry preview; physical mapping remains unknown.' };
    ...
    model.components = selected.map(([assetId, name, width, height, family], index) => { const component = componentFrom('plc', index + 1, 115 + (index % 4) * 320, index < 4 ? 820 : 560); component.id = `r4-component-${index + 1}`; component.tag = `-CAD${index + 1}`; component.name = name; component.partNumber = `CANDIDATE-${assetId}`; component.assetId = assetId; component.source_asset_id = assetId; component.width = width; component.height = height; component.mounting = index === 6 ? 'Plate' : 'DIN'; component.group = family; component.metadata = { geometryStatus: 'source_verified', identityStatus: 'candidate_needs_review', physicalMappingStatus: 'unknown', sourceAssetId: assetId, sourceUnits: 'unknown-or-source-units', placementCapable: false }; return component; });
    return model;
  }

  const scenarios = { starter: starterScenario, mcc: mccScenario, plc: plcScenario, r4: r4Scenario };
  const GRID_MM = 5;
  const state = { model: mccScenario(), selectedId: null, selectedAssetId: null, zoom: 1, panX: 0, panY: 0, grid: true, issues: [], drag: null, pan: null, assetCatalog: [], productCatalog: [], libraryMode: 'components' };
  ...
  function pointerDrop(event) { const point = pointerToModel(event); const assetId = event.dataTransfer?.getData('text/cnb-asset'); if (assetId) { const asset = state.assetCatalog.find((item) => assetKey(item) === assetId); if (asset) { const approved = assetApproved(asset); if (state.model.metadata?.authoritative && !approved) showToast('Chưa xác minh kích thước: không thể đưa vào authoritative layout', 'error'); else addAssetComponent(asset, approved, point); } event.preventDefault(); return; } const kind = event.dataTransfer?.getData('text/cnb-component'); if (kind) { addComponent(kind, point.x, point.y); event.preventDefault(); } }
  ...
  function wireEvents() {
    ...
    $('#librarySearch').addEventListener('input', (event) => { const query = event.target.value.toLowerCase(); $$('.library-item').forEach((item) => { item.hidden = !item.textContent.toLowerCase().includes(query); }); renderAssetLibrary(); }); $('#assetFilter').addEventListener('change', renderAssetLibrary);
    $('#componentTab').addEventListener('click', () => { state.libraryMode = 'components'; $('#componentTab').classList.add('active'); $('#assetTab').classList.remove('active'); $('#productTab').classList.remove('active'); $('#componentTab').setAttribute('aria-selected', 'true'); $('#assetTab').setAttribute('aria-selected', 'false'); $('#productTab').setAttribute('aria-selected', 'false'); $('#libraryList').hidden = false; $('#assetList').hidden = true; $('#productList').hidden = true; });
    $('#assetTab').addEventListener('click', () => { state.libraryMode = 'assets'; $('#assetTab').classList.add('active'); $('#componentTab').classList.remove('active'); $('#productTab').classList.remove('active'); $('#componentTab').setAttribute('aria-selected', 'false'); $('#assetTab').setAttribute('aria-selected', 'true'); $('#productTab').setAttribute('aria-selected', 'false'); $('#libraryList').hidden = true; $('#assetList').hidden = false; $('#productList').hidden = true; renderAssetLibrary(); });
    $('#productTab').addEventListener('click', () => { state.libraryMode = 'products'; $('#productTab').classList.add('active'); $('#componentTab').classList.remove('active'); $('#assetTab').classList.remove('active'); $('#componentTab').setAttribute('aria-selected', 'false'); $('#assetTab').setAttribute('aria-selected', 'false'); $('#productTab').setAttribute('aria-selected', 'true'); $('#libraryList').hidden = true; $('#assetList').hidden = true; $('#productList').hidden = false; renderProductLibrary(); });
    ...
    svg.addEventListener('dragover', (event) => { if (event.dataTransfer.types.includes('text/cnb-component') || event.dataTransfer.types.includes('text/cnb-asset')) event.preventDefault(); });
  ...
  renderLibrary(); wireEvents(); validate(); fitCanvas(); loadAssetCatalog(); fetch('/api/r3/products').then((response) => response.ok ? response.json() : null).then((payload) => { state.productCatalog = payload?.products || []; renderProductLibrary(); }).catch(() => {}); window.CNB_APP = { state, loadModel, autoLayout, validate, exportFormat, scenarios, zoomAt, fitCanvas, pointerToModel, render };
}());
