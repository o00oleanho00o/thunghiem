(function bootstrap() {
  ...
  function validate() {
    ...
    for (let a = 0; a < model.components.length; a += 1) for (let b = a + 1; b < model.components.length; b += 1) {
      const left = model.components[a]; const right = model.components[b]; if (pointInside(rect(left), rect(right))) issues.push({ code: 'E002', severity: 'error', entity: left.id, message: `${left.tag} overlaps ${right.tag}`, hint: 'Separate footprints and rerun layout.' });
    }
  ...
  function render() {
    ...
    svg.setAttribute('viewBox', `0 0 ${model.enclosure.width} ${model.enclosure.height}`); svg.style.transform = `translate(${state.panX}px, ${state.panY}px) scale(${state.zoom})`; svg.classList.toggle('dragging', Boolean(state.drag || state.pan));
  ...
  function wireEvents() {
    ...
    svg.addEventListener('pointerdown', (event) => {
      ...
      const point = pointerToModel(event); state.selectedId = id; state.drag = { id, dx: point.x - component.x, dy: point.y - component.y }; svg.setPointerCapture(event.pointerId); render();
    });
    ...
    svg.addEventListener('pointerup', (event) => {
      if (state.pan) { state.pan = null; svg.releasePointerCapture?.(event.pointerId); return; }
