(function attachCadExport(global) {
  ...
  function normalizeModel(source) {
    ...
    const components = (Array.isArray(source.components) ? source.components : []).map((c, i) => {
      ...
      return { id: c.id || `device-${String(i + 1).padStart(3, '0')}`, tag: c.tag || c.deviceTag || `-Q${i + 1}`, name: c.name || c.description || c.kind || 'Component', kind: c.kind || 'device', manufacturer: c.manufacturer || '', partNumber: c.partNumber || c.mpn || '', group: c.group || c.function || 'General', width: Math.max(1, number(c.width ?? fp.width, 40)), height: Math.max(1, number(c.height ?? fp.height, 30)), depth: Math.max(1, number(c.depth ?? fp.depth, 50)), x: number(c.x, plate.x + 30 + (i % 6) * 100), y: number(c.y, plate.y + plate.height - 100 - Math.floor(i / 6) * 100), rotation: [0, 90, 180, 270].includes(Number(c.rotation)) ? Number(c.rotation) : 0, railId: c.railId || null, locked: Boolean(c.locked), terminals: Array.isArray(c.terminals) ? c.terminals : [], mounting: c.mounting || 'DIN', color: c.color || '#3b82f6', assetId: c.assetId || c.source_asset_id || null, source_asset_id: c.source_asset_id || c.assetId || null, footprintRef: c.footprintRef || c.footprint_ref || null, footprint_ref: c.footprint_ref || c.footprintRef || null, metadata: c.metadata || {} };
    });
