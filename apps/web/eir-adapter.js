  ...
  function adapt(raw) {
    ...
    const components = devices.map((device, index) => {
      ...
      return {
        ...
        metadata: (() => { const provenance = part.provenance || {}; const clearance = provenance.clearance_mm || {}; const width = provenance.width_mm || {}; const height = provenance.height_mm || {}; const depth = provenance.depth_mm || {}; const mounting = provenance.mounting || {}; const artifacts = part.source_artifacts || part.sourceArtifacts || []; return { sourceEirId: deviceId, partId: first(device.partId, part.id, null), productIdentity: first(device.productIdentity, part.productIdentity, part.product_identity, null), sourceArtifacts: artifacts, widthProvenance: width, heightProvenance: height, depthProvenance: depth, mountingProvenance: mounting, clearanceMm: first(device.clearanceMm, part.clearanceMm, part.footprint?.clearance_mm, part.footprint?.clearanceMm, 0), clearanceStatus: first(clearance.status, part.clearance_status, null), clearanceArtifactId: first(clearance.source_artifact_id, null), clearanceSourceLocator: first(clearance.source_locator, null), serviceAccessDirection: first(device.serviceAccessDirection, part.serviceAccessDirection, part.footprint?.service_access_direction, part.footprint?.serviceAccessDirection, null), serviceAccessDepthMm: first(device.serviceAccessDepthMm, part.serviceAccessDepthMm, part.footprint?.service_access_depth_mm, part.footprint?.serviceAccessDepthMm, null), terminalModelStatus: first(part.terminal_model_status, part.terminalModelStatus, 'unknown') }; })(),
      };
    });
